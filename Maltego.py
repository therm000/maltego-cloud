...
class MaltegoEntity(object):
	...
	def returnEntityString(self):
		s = ''
		s += "<Entity Type=\"" + str(self.entityType) + "\">" + '\n';
		s +=  "<Value>" + str(self.value) + "</Value>"+ '\n'
		s +=  "<Weight>" + str(self.weight) + "</Weight>"+ '\n'
		if (self.displayInformation is not None):
			s +=  "<DisplayInformation><Label Name=\"\" Type=\"text/html\"><![CDATA[" + str(self.displayInformation) + "]]></Label></DisplayInformation>"+ '\n'
		if (len(self.additionalFields) > 0):
			s +=  "<AdditionalFields>"+ '\n'
			for i in range(len(self.additionalFields)):
				if (str(self.additionalFields[i][2]) <> "strict"):
					s +=  "<Field Name=\"" + str(self.additionalFields[i][0]) + "\" DisplayName=\"" + str(self.additionalFields[i][1]) + "\">" + str(self.additionalFields[i][3]) + "</Field>"+ '\n'
				else:
					s +=  "<Field MatchingRule=\"" + str(self.additionalFields[i][2]) + "\" Name=\"" + str(self.additionalFields[i][0]) + "\" DisplayName=\"" + str(self.additionalFields[i][1]) + "\">" + str(self.additionalFields[i][3]) + "</Field>"+ '\n'
			s +=  "</AdditionalFields>"+ '\n'
		if (len(self.iconURL) > 0):
			s +=  "<IconURL>" + self.iconURL + "</IconURL>"+ '\n'
		s += "</Entity>"+ '\n'
		return s
...
class MaltegoTransform(object):
	...
	def returnOutputString(self):
		s = ''		
		s += "<MaltegoMessage>" + '\n';
		s += "<MaltegoTransformResponseMessage>" + '\n';
						
		s +=  "<Entities>" + '\n'
		for i in range(len(self.entities)):
			s += self.entities[i].returnEntityString();
		s +=  "</Entities>"+ '\n'
						
		s +=  "<UIMessages>"+ '\n'
		for i in range(len(self.UIMessages)):
			s +=  "<UIMessage MessageType=\"" + self.UIMessages[i][0] + "\">" + self.UIMessages[i][1] + "</UIMessage>" + '\n';
		s +=  "</UIMessages>" + '\n'
